...
public class PlanApurementEchelonnementPMTest extends PlanApurementAbstract {

    private static List<String> listBce = new ArrayList<>();

    @DataProvider(name = "data")
    public Iterator<Object[]> data() {
        return getDataProvider("TA_PlanApurement.xlsx", "Plan d'apurement Echelonnement", "Plan d'apurement", "PA PM", null);
    }
    ...
    private String selectNissOrBce() {
        String nissOrBce;
        if (listBce.isEmpty()) {
            listBce = DaoFunctionality.getNascaDao().getBceForPlanApurement(getPeriode().substring(0, 4));
        }
        nissOrBce = listBce.get(0);
        listBce.remove(0);
